package br.com.desafio.qa.web.support;
...
public class PageObject {
	...
	public Boolean VerificaObjetoExistente(By Type, WebElement elemento, Boolean Obrigatory, int tentativas)
	...
	// função para preenchimento de campos por tipo de elemento

	public Boolean PreencheValorCampoSetSelectButton(By Type, WebElement element_view, String valor, int tentativas)
			throws InterruptedException {
		for (int i = 0; i < tentativas; i++)
			try {
				if (VerificaObjetoExistente(Type, element_view, true, 1)) {
					...
					if (htmltag.toUpperCase().trim().equals("INPUT")
							&& type_element.toUpperCase().trim().equals("IMAGE"))
						htmltag = "BUTTON";

					if (htmltag.toUpperCase().trim().equals("INPUT")
							&& type_element.toUpperCase().trim().equals("BUTTON"))
						htmltag = "BUTTON";

					if (htmltag.toUpperCase().trim().equals("INPUT")
							&& type_element.toUpperCase().trim().equals("CHECKBOX"))
						htmltag = "BUTTON";

					if (htmltag.toUpperCase().trim().equals("INPUT")
							&& type_element.toUpperCase().trim().equals("RADIO"))
						htmltag = "BUTTON";
